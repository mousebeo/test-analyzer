    ...
    <div className="space-y-6">
        <div>
            <h3 className="text-xl font-semibold text-indigo-600 mb-2">
                Detailed Thread Analysis <span className="text-sm font-normal text-gray-400">(Local Analysis)</span>
            </h3>
            <p className="text-sm text-gray-600 italic">{report.summary}</p>
        </div>

        {report.warnings && report.warnings.length > 0 && (
            <div>
                <h4 className="font-semibold text-gray-600 mb-2 text-sm uppercase tracking-wider">Warnings</h4>
        ...
        {report.problematicThreads && report.problematicThreads.length > 0 && (
            <div>
                <h4 className="font-semibold text-gray-600 mb-2 text-sm uppercase tracking-wider">Problematic Threads</h4>
                <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                    {report.problematicThreads.map((thread, i) => (
                        <div key={i} className="bg-gray-100 p-4 rounded-lg">
                            ...
                            <p className="text-sm text-gray-600"><span className="font-semibold">State:</span> {thread.state}</p>
                            <p className="text-sm text-gray-600"><span className="font-semibold">Details:</span> {thread.details}</p>
                            {thread.stackTraceSnippet && (
                                <details className="mt-2">
                                    <summary className="cursor-pointer text-sm text-indigo-600 hover:text-indigo-500">Show Stack Trace Snippet</summary>
                                    <pre className="mt-2 bg-gray-800 p-3 rounded text-xs text-gray-200 overflow-x-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div className="space-y-4">
                ...
                <div className="bg-gray-100 p-4 rounded-md">
                    <h4 className="font-semibold text-red-600">Deadlocked Threads</h4>
                    {(threadData.deadlockedThreads || []).length > 0 ? (
                        ...
                        <p className="text-green-600 mt-1">None detected.</p>
                    )}
                </div>
            </div>
