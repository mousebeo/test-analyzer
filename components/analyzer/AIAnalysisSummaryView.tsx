...
const AreaOfConcern: React.FC<{ concern: ApplicationWarning }> = ({ concern }) => {
    const severityClasses = {
      High: { iconColor: 'text-red-400', textColor: 'text-red-300', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /> },
      Medium: { iconColor: 'text-yellow-400', textColor: 'text-yellow-300', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /> },
      Low: { iconColor: 'text-sky-400', textColor: 'text-sky-300', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /> },
    };
...
export const AIAnalysisSummaryView: React.FC<AIAnalysisSummaryViewProps> = ({ result, onDeeperAnalysis, isAnalyzingDeeper, deeperAnalysisError, onSaveToKB, isSavingToKB, saveToKBSuccess, saveToKBError }) => {
    ...
    if (!aiSummary) {
        return <div className="text-center text-gray-400">AI Summary is not available.</div>;
    }
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-gray-800 p-6 rounded-lg">
                    <h3 className="text-lg font-semibold text-gray-200 mb-4">Health Highlights</h3>
                    <div className="space-y-3">
                        {aiSummary.healthHighlights.map((text, i) => <HealthHighlight key={i} text={text} />)}
                    </div>
                </div>
                 <div className="bg-gray-800 p-6 rounded-lg">
                    <h3 className="text-lg font-semibold text-gray-200 mb-4">Areas of Concern</h3>
                    <div className="space-y-3">
                        {aiSummary.areasOfConcern.length > 0 ? (
                           aiSummary.areasOfConcern.map((concern, i) => <AreaOfConcern key={i} concern={concern} />)
                        ) : (
                            <p className="text-gray-400 italic">No major concerns identified by AI.</p>
                        )}
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-gray-800 p-4 rounded-lg">
                    <h3 className="font-semibold text-lg text-gray-200 mb-4 text-center">Top 5 Active Processes (by Jobs Created)</h3>
                 ...
                 <div className="bg-gray-800 p-4 rounded-lg">
                    <h3 className="font-semibold text-lg text-gray-200 mb-4 text-center">Top 5 Longest Activities (by Max Time)</h3>
                        ...
                        <div style={{height: 300}}>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={result.topActivitiesByTime} layout="vertical" margin={{ left: 100 }}>
                                    <CartesianGrid strokeDasharray="3 3" stroke="#4A5568" />
                                    <XAxis type="number" stroke="#A0AEC0" fontSize={12} label={{ value: 'ms', position: 'insideBottomRight', offset: -5, fill: '#A0AEC0' }}/>
                                    <YAxis type="category" dataKey="name" stroke="#A0AEC0" width={150} fontSize={10} interval={0} />
                                    <Tooltip content={<CustomTooltip />} cursor={{fill: '#374151'}}/>
            ...
            <div className="mt-8 border-t border-gray-700 pt-6">
                <div className="text-center bg-gray-800 p-6 rounded-lg">
                    <h3 className="text-lg font-semibold text-gray-200">Ready for a Deeper Look?</h3>
                    <p className="text-sm text-gray-400 my-3 max-w-xl mx-auto">
                    ...
                    <button
                        ...
                        className="bg-sky-600 hover:bg-sky-700 disabled:bg-gray-600 text-white font-bold py-2 px-5 rounded-lg transition-colors shadow-md hover:shadow-sky-500/50 flex items-center justify-center mx-auto"
                    >
                         {isAnalyzingDeeper ? (
                            <>
                    ...
                    {deeperAnalysisError && (
                        <p className="text-red-400 text-sm mt-3">{deeperAnalysisError}</p>
                    )}
                </div>
            </div>
        </div>
